...
- Large diff or truncated output: Use `--stat` first, then run `git diff target..current -- filepath` for each modified file individually to capture complete changes
